...
int bag_rule_compar(void const *a, void const *b) {
	bag_rule const *pa = a;
	bag_rule const *pb = b;
	
	return pa->container_id - pb->container_id;
}
...
//Assumes rules are sorted
int bags_within(int id, VECTOR_PTR_PARAM(bag_rule, rules)) {
	bag_rule key = {.container_id = id};
	bag_rule *r = bsearch(&key, *rules, *rules_len, sizeof(bag_rule), bag_rule_compar);
	if (!r) {
		puts("Error! This bag does not have a rule!");
	}
	
	int count = 1;
	int i;
	for (i = 0; i < r->num; i++) {
		count += r->allowed_amt[i] * bags_within(r->allowed[i], VECTOR_ARG(*rules));
	}
	return count;
}
...
int main() {
	...
	if (!id) {
		puts("Shiny gold bags contain nothing");
	} else {
		//Since we've already gone to the trouble of finding the
		//transitive closure, let's make sure shiny gold bags don't
		//contain themselves, or any bag that goes on to contain itself
		char *row = adj + *id * n;
		int i;
		for (i = 0; i < n; i++) {
			if (row[i] && adj[n*i + i]) {
				puts("Error! Some bags contain themselves!");
				goto done;
			}
		}
		
		//Okay, now we'll do the actual task we were asked to do using
		//a sort of DFS. To make things a little faster, we'll sort the
		//rule list by container_id then bsearch it
		qsort(rules, rules_len, sizeof(bag_rule), bag_rule_compar);
		
		printf("Shiny gold bags contain %d bags\n", bags_within(*id, VECTOR_ARG(rules)) - 1);
