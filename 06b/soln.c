...
int main() {
	...
	//For each group, get sum of cardinality of intersection set of all people
	int i;
	int count = 0;
	for (i = 0; i < grps_len; i++) {
		...
		while(*tmp) {
			needle[0] = *tmp;
			int howmany = strspn(tmp, needle);
			
			//Assumes no person answered the same question more than once
			if (howmany == grps[i].people_len) all_count++;
